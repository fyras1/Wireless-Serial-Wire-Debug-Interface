...
//		GPIOD->ODR&=~SWD_MASTER_CLk_Pin; // 0
	...
	gpio_set_level(SWD_MASTER_DATA_Pin, 0);
...
inline uint8_t readAck()
{
	 ...
	 //  GPIOD->MODER&= ~(1<<26); // DATA PIN INPUT MODE
	   ...
	   gpio_set_direction(SWD_MASTER_DATA_Pin, GPIO_MODE_INPUT);
	...
	//	GPIOD->MODER|=(1<<26); // DATA PIN OUTPUT MODE
		gpio_set_direction(SWD_MASTER_DATA_Pin, GPIO_MODE_OUTPUT);
...
inline void swdio_mode_input(){
	//GPIOD->MODER&= ~(1<<26); // DATA PIN INPUT MODE
   	...
   	gpio_set_direction(SWD_MASTER_CLK_Pin, GPIO_MODE_INPUT);
...
inline void swclk_reset(){
	...
	gpio_set_level(SWD_MASTER_CLK_Pin,0);
}
...
gpio_set_level(SWD_MASTER_DATA_Pin,bit);
