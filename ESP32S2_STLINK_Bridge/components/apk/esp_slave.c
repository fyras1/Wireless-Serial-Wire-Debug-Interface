			...
			//	HAL_GPIO_WritePin(GPIOF, GPIO_PIN_12, 1);
				...
				if (retAckWait) //inside ack wait (return ack wait)
							...
							switch(retAckWait) //0 -> 1 -> 0
								{
								case 2:  { gpio_set_level(SWD_SLAVE_DATA_Pin, 1);pinState =1; break;}
								default:  { gpio_set_level(SWD_SLAVE_DATA_Pin,0); pinState =0; break;}
								}
							}
				else
							{
							switch(retAckOk) //1 -> 0 -> 0
								{
								case 1:    { gpio_set_level(SWD_SLAVE_DATA_Pin, 1);pinState =1; break;}
								default:  { gpio_set_level(SWD_SLAVE_DATA_Pin, 0);pinState =0; break;}
								}
							}
		...
		case SWD_TURNAROUND_RQ_ACK:	// Switch trigger on next rising edge to FALLING
			{
				...
				gpio_set_direction(SWD_SLAVE_DATA_Pin, GPIO_MODE_OUTPUT);
				changeEdgeTrigger(RISING);
		...
		case SWD_TURNAROUND_DAT_RQ:	// if  READ request : Switch trigger and skip next RISING edge
			{
				...
				gpio_set_level(SWD_SLAVE_DATA_Pin, 1);
				//GPIOE->MODER&= ~(1<<18); // DATA PIN INPUT MODE
				gpio_set_direction(SWD_SLAVE_DATA_Pin, GPIO_MODE_INPUT);
		...
		case RISING:
			{
				...
				gpio_set_intr_type(SWD_SLAVE_CLK_Pin, GPIO_INTR_POSEDGE);
		...
		case FALLING:
			{
				/*EXTI->FTSR |= (1 << 11);	//
				EXTI->RTSR &= ~(1 << 11);	//*/
				gpio_set_intr_type(SWD_SLAVE_CLK_Pin, GPIO_INTR_NEGEDGE);
